...
public class A08_FullCalController {
	...
	public ResponseEntity<CalResult> insertCalendar(Calendar ins) {
		return ResponseEntity.ok(service.insertCalendar(ins));
	}	
	@RequestMapping("calendarUpdate.do")
	public ResponseEntity<CalResult> calendarUpdate(Calendar upt) {
		return ResponseEntity.ok(service.uptCalendar(upt));
	}	
	// http://localhost:7080/springweb/calendarDelete.do?id=9
	@RequestMapping("calendarDelete.do")
	public ResponseEntity<CalResult> calendarDelete(@RequestParam("id") int id) {
