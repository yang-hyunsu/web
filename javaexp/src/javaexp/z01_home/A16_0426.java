...
//      1) 예외 처리를 왜 하는지 알고 있나요?
      /*
       개발자의 입력이나 사용자의 사용으로 인해서 발생하는 에러들을 효율적으로 발견하여 처리하기위함이다.
       더 나아가 프로그램 자체가 멈추는 상황에 미리 대응하여 에러가 생겨도 프로그램이 멈추지 않도록 하는것이 목표이다.
       */
...
//      2) 예외 처리를 하는 기본 단계별 구조를 설명할 수 있고, 코드할 수 있는지?
      ...
      // 다양한 에러처리를 할 때에는 계층적으로 최하위-> 차상위->최상위 순으로 처리한다
   }
      ...
      3단계
         ...
         try {
           ...
           // 예외가 위임된 후에 호출할 내용을 try 블럭안에 넣는다
           // 메소드 별로 위임된 예외들이 처리되며 던져진 메소드에 따라 처리될 catch 블럭이 다르다.
        
        4단계
         class 사용정정의 예외 클래스 extends Exception{
          사용자정의생성자(){
             super("예외");
          }
       ...
       // Exception을 상속받은 사용자 정의 클래스를 생성하여, 사용자가 정의한 에러를 잡고 처리할 수 있다.
    main()
       try{
          throw new 사용자정의예외();
       } catch(사용자정의예외) {
        ...
        // main()에서 try블럭내, throw 뒤에 객체를 생성하여 문제코드를 잡을 수 있도록한다.
        // catch에서 오버라이딩한 메소드를 불러와 처리내용을 사용자가 원하는대로 정의할 수있다.
       */
//            3) args값이 입력하지 않을 때, 처리되는 메서드를 선언하고, 예외를 위임하세요.
      try {
            call(args[0]);
...
//          2. border의 유형에 따른 옵션
//              border-style: dotted, solod, double, dashed
      ...
      /*
         ...
         <body>
             <div>
                 ...
                 border-radius: 20px;">버튼1</span>
                 <span style="
                 background: blue;
                 border: 2px black solid; 
                 border-radius: 20px 30px 50px;">버튼1</span>
             </div>
         </body>
         </html>
