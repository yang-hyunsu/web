...
public class A11_TypeChange {
	...
	public static void main(String[] args) {
		...
		*/
		byte num01 = 30;
		int num02 = 230;
		// 주의 : 데이터는 오른쪽에서 =(대입연산자)에 의해서
		//	왼쪽으로 할당된다.
		//	순수데이터로 할당할 수 있는것이 아니고, 그것을
		...
		// num01 = num02;(x) 
		num01 = (byte)num02; 
		// bit 단위로 casting 되기에 연산결과가 예측이 힘들다.
		// 010101000 8bit
		// 0101 4bit
		// 0100일 잘려지는 효과가 있다.
		
		// 큰 용기에 있는 데이터를 작은 용기에
		// 있는 데이터로 할당,
		// 1. 데이터 할당 불가(자동형변환이 불가)
		// 2. 큰 용기에 있는 데이터를 작은 용기에 할당하려면
		// 	  casting작업이 필요하다.
		//	  (byte) 해당 틀을 지정하여 처리
		// 
		...
		*/	
		...
		1. 자바는 정수/정수형은 정수형으로 처리된다.
			소숫점이하를 처리해주지 않는다.
			=> 소숫점이하를 처리하기 위해서는 나눗셈의
			데이터 중 적어도 하나는 실수형으로 형변환하여
			처리하여야 한다.(강제형변환-casting)
			cf) casting을 무조건 큰 데이터를 작은 데이터에
			할당하는 것이 아니라, 데이터 유형의 크기/종류에 
			상관없이 강제 형변환시 사용한다.
			** 반드시 기억!!!!!!!!!!!!
		*/
		int num07 = 10;
		...
		System.out.println((double)num07/(double)num08); //실수
		// ex) 25와 7을 정수로 선언하여,
		//	   소숫점이하가 나오지 않는 경우와
